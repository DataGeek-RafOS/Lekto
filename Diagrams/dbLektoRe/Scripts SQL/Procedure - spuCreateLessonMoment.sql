...
CreateLessonMoment: BEGIN
	...
	IF in_Debugging = 1
	THEN	
		/*NAME:tabLessonMoment*/		SELECT 'tabLessonMoment', idLessonMoment, dtProcessed FROM tabLessonMoment;		
	END IF;	
	...
	IF in_Debugging = 1
	THEN	
		/*NAME:tabStudentGroup*/		SELECT 'tabStudentGroup', idStudent, nuGroup			
		FROM tabStudentGroup
		ORDER BY nuGroup ASC, idStudent ASC;		
	END IF;	
	...
	SELECT LeMo.idNetwork
		...
		, Acti.nuOrder
		, LeMo.idLessonMoment
		, CASE WHEN LeSt.nuOrder IN (1, 3) -- Todos os alunos participam do passo 1 e 3
			  THEN 0
			  ELSE ROW_NUMBER() OVER ( PARTITION BY Less.idLesson, LeSt.idLessonStep ORDER BY RAND() ) -- A distribuicao ocorre nas atividades do passo 2
		  END AS nuGroup
	FROM LessonMoment LeMo
		INNER JOIN LessonTrackGroup LtGr
			   ON LtGr.idLessonTrackGroup = LeMo.idLessonTrackGroup
	...
	IF in_Debugging = 1
	THEN	
	/*NAME:tabMomentActivity*/
		SELECT 'tabMomentActivity'
			, idLesson		
			, idLessonStep		
			, nuOrderStep		
			, idLessonActivity	
		...
		ORDER BY idLessonMoment, nuOrderStep, nuOrderActivity;		
	END IF;		
     ...
     START TRANSACTION;	
          ...
          INSERT INTO LessonMomentActivity
               ...
               SELECT idNetwork
                    ...
                    , NOW()
               FROM tabMomentActivity
               WHERE nuGroup <= 3; -- Uma das atividades é descartada
		...
		SELECT Scla.idNetwork
			, Scla.idSchool
			, Scla.coGrade
			, Scla.idSchoolYear
			, Scla.idClass
			, tMoA.idLessonActivity
		...
		IF in_Debugging = 1
		THEN	
			/*NAME:LessonMomentGroup*/			SELECT 'LessonMomentGroup'
				, Scla.idUserStudent
				, Scla.idStudent
				, tMoA.idLessonActivity				
				, Lmac.idLessonMomentActivity
